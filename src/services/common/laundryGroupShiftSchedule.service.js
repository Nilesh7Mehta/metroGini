  ...
  const { rows } = await client.query(
    `SELECT s.shift_name
     ...
     WHERE COALESCE(s.status, TRUE) IS TRUE
       AND NOT EXISTS (
         SELECT 1
         FROM laundry_group_shift_schedule t
...
const assertShiftSlotsAvailable = async (entries, client) => {
  ...
  throw {
    status: 400,
    message: await buildConflictMessage(conflicts, client, 'merchant'),
  };
};
...
export const mapShiftScheduleError = async (err) => {
  if (err.code === '23505') {
    const isScheduleConstraint =
      err.constraint === 'uq_lgss_group_day_shift' ||
    ...
    if (parsed) {
      ...
      return {
        status: 400,
        message: `${dayLabel} (${groupLabel}): ${shiftLabel} already assigned to another merchant. ${buildAvailabilityNote(availableNames)}`,
      };
    }
