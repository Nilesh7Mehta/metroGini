...
/** User-facing stain charge = vendor share + markup (e.g. small 50 + 25 = 75). */
const resolveUserStainCharge = (order) => {
  if (!order?.is_stained) return 0;
  const vendorShare = Number(order.vendor_request_amount || 0);
  const markup = Number(order.vendor_request_markup || 0);
    ...
    pricing: {
      ...
      base_total:
        order.final_total != null
          ? parseFloat((Number(order.final_total) - userStainCharge).toFixed(2))
          : null,
